...
* classificationType ^definition = "It represents the classification system to define the stage, such as TNM (Tumor Linphonode Metastasis)."
* value 1..* Element "Value"
* value ^definition = "It represents the stage value. Its datatype and cardinality depend on the specific structure of the staging classification being used. For example, in the case of the TNM classification, three string values are expected: one for T (Tumor), one for N (Nodes), and one for M (Metastasis)."
